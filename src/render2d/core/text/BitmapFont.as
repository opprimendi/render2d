...
package render2d.core.text
{
	...
	import render2d.core.text.cleanMasterString;
    ...
    public class BitmapFont
    {
        ...
        private var mFontMaterial:TextureBase;
        ...
        private function parseFontXml(fontXml:XML):void
        {
            ...
            mSize = Number(fontXml.info.@size);
            mLineHeight = Number(fontXml.common.@lineHeight);
            mBaseline = Number(fontXml.common.@base);

            if (mSize <= 0)
            {
                trace("Warning: invalid font size in '" + mName + "' font.");
            ...
            for each (var charElement:XML in fontXml.chars.char)
            {
                ...
                var xOffset:Number  = Number(charElement.@xoffset);
                var yOffset:Number  = Number(charElement.@yoffset);
                var xAdvance:Number = Number(charElement.@xadvance);

              	var regionX:Number= Number(charElement.@x) + frameX;
              	var regionY:Number= Number(charElement.@y) + frameY;
              	var regionWidth:Number= Number(charElement.@width);
              	var regionHeight:Number= Number(charElement.@height);
            ...
            {
                var first:int  = int(kerningElement.@first);
                var second:int = int(kerningElement.@second);
                var amount:Number = Number(kerningElement.@amount);
                                      ...
                                      kerning:Boolean=true):void
        {
            ...
            for (var i:int=0; i<numChars; ++i)
            {
				...
				width = char.width * scale;
				height = char.height * scale;

				var x:Number = charLocation.x - geometryWidth / 2;
				var y:Number = charLocation.y - geometryHeight / 2;
				
				var u1:Number = char.x / textureWidth;
				var u2:Number = (char.x + char.width) / textureWidth;
				var v1:Number = char.y / textureHeight;
				var v2:Number = (char.y + char.height) / textureHeight;
        ...
        public function get fontMaterial():TextureBase { return mFontMaterial; }
    }
}

import render2d.core.text.BitmapChar;
